...
const CartProgressBar = ({
  ...
  const sortedOffers = offers.sort((a, b) => {
    const goalA =
      a.goalType === "quantity"
        ? parseFloat(a.goalquantity || 0)
        : parseFloat(a.goalAmount || 0);
    const goalB =
      b.goalType === "quantity"
        ? parseFloat(b.goalquantity || 0)
        : parseFloat(b.goalAmount || 0);
    return goalA - goalB;
  });
  ...
  const calculateProgress = () => {
    ...
    for (let i = 0; i < sortedOffers.length; i++) {
      ...
      const goal = isQuantity
        ? parseFloat(offer.goalquantity || 0)
  ...
  const getDynamicText = () => {
    ...
    if (!activeOffer) {
      ...
      const rewardDescription =
        lastOffer.rewardType === "discount"
          ? `${lastOffer.discountCode || 0}${lastOffer.discountType === "percentage" ? "%" : "$"} Discount`
          : lastOffer.rewardType === "shipping"
            ? "Free Shipping"
            : lastOffer.rewardType === "gift"
              ? "Free Gift"
              : "Reward";
    ...
    const goal = isQuantity
      ? parseFloat(activeOffer.goalquantity || 0)
      : parseFloat(activeOffer.goalAmount || 0);
    const current = isQuantity ? itemCount : cartValue;
    const amountLeft = Math.max(0, goal - current);

    const rewardDescription =
      activeOffer.rewardType === "discount"
        ? `${activeOffer.discountCode || 0}${activeOffer.discountType === "percentage" ? "%" : "$"} Discount`
        : activeOffer.rewardType === "shipping"
          ? "Free Shipping"
          : activeOffer.rewardType === "gift"
            ? "Free Gift"
            : "Reward";

    const unit = isQuantity ? "item" : "$";
    const amountText = isQuantity
      ? `${amountLeft} more item${amountLeft !== 1 ? "s" : ""}`
      : `$${amountLeft}`;

    return `Add ${amountText} to unlock ${rewardDescription}`;
  };
  ...
  return (
    <div className={`cart-progress-bar ${placement}`}>
      ...
      <div className="relative w-full mb-6">
        ...
        {/* Milestones */}
        <div
          className={`${isMobile ? "flex flex-col space-y-4" : "absolute top-0 w-full"}`}
        >
          {sortedOffers.map((offer, index) => {
            ...
            return (
              <div
                key={offer.id}
                className={`${isMobile ? "flex items-center space-x-3" : "absolute"}`}
              >
                {/* Milestone Circle */}
                <div
                  className={`
                ...
                {/* Milestone Info */}
                <div
                  className={`${isMobile ? "flex-1" : "absolute"} ${isMobile ? "" : "top-12"} ${isMobile ? "" : "left-1/2"} ${isMobile ? "" : "transform -translate-x-1/2"}`}
                >
                  {/* Goal Label */}
                  ...
                  {isCompleted &&
                    ...
                    offer.rewardProducts.length > 0 && (
                      <div
                        className={`mt-2 ${isMobile ? "flex space-x-2" : "flex flex-col space-y-1"}`}
                      >
                        {offer.rewardProducts
                            ...
                            >
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  alert(
                                    `Preview: Would add "${product.title}" to cart`,
                                  );
                                }}
