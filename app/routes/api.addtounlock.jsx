...
// Loader to ensure authentication
...
export const action = async ({ request }) => {
  try {
    ...
    if (!session || !session.shop || !session.accessToken) {
      console.error("❌ Authentication failed: Missing session data");
      return json(
        {
          success: false,
    ...
    const progressBarStyle = JSON.parse(
      formData.get("progressBarStyle") || "{}",
    );
    // Handle placement as string (not JSON) - it can be "home", "Page", or "cart"
    const placement = formData.get("placement") || "";

    // Block products data
    const blockProducts = JSON.parse(formData.get("blockProducts") || "[]");
    const blockProductSelectionType =
      formData.get("blockProductSelectionType") || "all";
    const blockProductCollections = JSON.parse(
      formData.get("blockProductCollections") || "[]",
    );

    // --- Create Campaign
    // Prepare placement data - store as object with placement string and block products metadata
    const placementData = {
      placement: placement,
      blockProducts: blockProducts,
      blockProductSelectionType: blockProductSelectionType,
      blockProductCollections: blockProductCollections,
    };

    console.log("📝 Creating campaign with placement:", placement);
    console.log("📝 Block products:", blockProducts.length);
    ...
    console.log("✅ Created campaign:", {
      ...
      placement: upsellCampaign.placement,
    });
    ...
    // Map to track created offers: frontend offer.id -> database createdOffer.id
    const createdOffersMap = new Map();
    ...
    for (const offer of offers) {
      ...
      // Store mapping: frontend offer.id -> database createdOffer.id
      createdOffersMap.set(offer.id, createdOffer.id);
    ...
    // Create discounts using the discount extension for discount-type offers
    // const { createDiscountsWithExtension } = await import("./utils/discount_extension.server");
    // const discountResult = await createDiscountsWithExtension(
    //   shop,
    //   accessToken,
    //   offers,
    //   createdOffersMap,
    ...
    //   console.error("Failed to create discounts:", discountResult.error);
    // }

    // For non-discount offers (gift, shipping), use the existing add_to_unlock_ function
    const nonDiscountOffers = offers.filter(
      (offer) => offer.rewardType !== "discount",
    );

    if (nonDiscountOffers.length > 0) {
...
//       // Homepage: block products, reward products (NOT trigger products)
//       const homeProducts =
...
//                   // Product Page: trigger products, block products, reward products
//                   let productsToShow = [];
//                   if (offer.rewardType === "gift") {
//                     // For gift rewards, show reward products
//                     productsToShow = offer.rewardProducts || [];
//                   } else {
//                     // For discount/shipping, show block products if available, otherwise trigger products
...
//                 {/* Hide Block Products section for Cart placement - Cart uses trigger products only */}
//                 {placement !== "cart" && (
//                   <Card>
//                     <BlockStack gap="200">
//                       <Text as="h2" variant="headingMd" fontWeight="bold">
...
//                         <Banner tone="info">
//                           <Text>
//                             Block products are not used for Cart page placement.
//                             Cart page uses trigger products.
//                           </Text>
//                         </Banner>
//                       )}
//                       {!placement && (
