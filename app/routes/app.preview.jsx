...
import { Page, Card, Text, BlockStack, Thumbnail, Box, InlineStack, Button, Icon } from "@shopify/polaris";
...
export async function loader({ request }) {
    ...
    const progressBarStyle = JSON.parse(url.searchParams.get("progressBarStyle") || "{}");
            ...
            >
                <div
                    style={{
                        ...
                        height: "100%",
                        backgroundColor:
                            progress >= 100 ? style.goalCompleteColor || "#FF9800" : style.primaryColor || "#4CAF50",
...
export default function PreviewPage() {
    ...
    return (
        <Page title="Offer Preview" fullWidth backAction={{ content: "back", url: "/app" }}>
            <BlockStack gap="400">
                <Card sectioned>
                    <BlockStack gap="400">
                        <InlineStack gap={"200"}>
                        ...
                        <Text>Placement: {placement.length > 0 ? placement.join(", ") : "None"}</Text>
                        ...
                        <BlockStack gap="400">
                            {products.map((p) => (
                                <BlockStack key={p.id} gap="200">
                                    <Thumbnail source={p.media || ""} alt={p.title || "Product"} />
                            ...
                            {offers.map((offer, index) => {
                                ...
                                return (
                                    <BlockStack key={offer.id} gap="200">
                                        ...
                                        <Text>Goal Type: {offer.goalType === "amount_cart" ? "Cart Value" : "Product Quantity"}</Text>
                                        <Text>Goal: {goal}</Text>
                                        ...
                                        />
                                        <Text>
                                            Before Goal: {offer.goalTextBefore
                                                .replace("{{amount_left}}", amountLeft)
                                                .replace("{{reward}}", rewardDescription)
                                                .replace("{{goal}}", goal)}
                                        </Text>
                                        <Text>
                                            After Goal: {offer.goalTextAfter
                                                .replace("{{reward}}", rewardDescription)
                                                .replace("{{goal}}", goal)}
                                        </Text>
                                            ...
                                            <BlockStack gap="200">
                                                ...
                                                {offer.rewardProducts.map((rp) => (
                                                    <BlockStack key={rp.id} gap="100">
                                                        <Thumbnail source={rp.media || ""} alt={rp.title || "Reward Product"} />
                                        ...
                                        {offer.rewardCollection?.length > 0 && (
                                            <BlockStack gap="200">
                                                ...
                                                {offer.rewardCollection.map((rc) => (
                                                    <Text key={rc.id}>{rc.title || "Untitled Collection"}</Text>
                                                ))}
                                            </BlockStack>
                                        )}
                                    </BlockStack>
