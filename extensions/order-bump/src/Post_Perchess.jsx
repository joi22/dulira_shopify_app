...
export default reactExtension("purchase.thank-you.cart-line-item.render-after", () => <App />);
...
function App() {
  ...
  const [post_perchess, setPost_Perchess] = useState(null)
  ...
  const domain = "https://threat-date-moderate-listing.trycloudflare.com";
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log(api, "this My api ")
        const response = await fetch(api);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        console.log("Fetched API data Post perchess:", data);
        setApiData(data);
      } catch (err) {
  ...
  return (
    ...
    //     </BlockStack>
    //   )}
    //   {latestOffer.campaign.type === "post_purchase" && rewardProducts.length > 0 && (
    //     <BlockStack spacing="tight" border="base" padding="loose">
    //       <Text size="medium" emphasis="bold">
    //         Thanks for your order! 🎉
    //       </Text>
    //       <Text tone="subdued">
    //         Before you go, here’s an exclusive deal you can still add:
    //       </Text>
    ...
    //               </Text>
    //             </BlockStack>
    //             <Button onPress={() => handleAddToOrder(reward)}>Add This Deal</Button>
    //           </InlineStack>
    //         </BlockStack>
    //       ))}
    //     </BlockStack>
